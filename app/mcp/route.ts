...
export async function POST(request: NextRequest) {
  ...
  if (rpc.method === "initialize") return result(rpc.id, { protocolVersion: rpc.params?.protocolVersion ?? "2025-03-26", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "agm-codex-onboarding", version: "0.2.2" }, instructions: "Use this server to finish AGM Codex onboarding. Never ask for passwords, tokens, recovery codes, or browser-session data." });
...
export async function GET() {
  ...
  return NextResponse.json({ name: "AGM Codex Onboarding MCP", version: "0.2.2", transport: "streamable-http", tools: tools.map((tool) => tool.name) }, { headers });
}
